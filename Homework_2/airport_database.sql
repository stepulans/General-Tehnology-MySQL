...
-- please scroll down, work with databes are on the bottom
USE airport;

CREATE TABLE airliners (
  id VARCHAR(128) PRIMARY KEY,
  model_name VARCHAR(128),
  flight_range INTEGER, -- error Проблема заключается в том, что range - это зарезервированное слово в MySQL и не может быть использовано в качестве имени столбца.
...
SELECT
	model_name,
	CASE
		WHEN flight_range > 1000 AND flight_range <= 2500
			THEN 'short_haul'
		WHEN flight_range > 2500 AND flight_range <= 6000
			THEN 'medium-haul'
		WHEN flight_range > 6000
			THEN 'long-haul'
	END AS category
FROM airliners;
...
SELECT
	model_name,
    CASE
		WHEN IFNULL(flight_range, 0) <= 1000
			THEN 'UNDEFINED'
		WHEN IFNULL(flight_range, 0) > 1000 AND IFNULL(flight_range, 0) <= 2500
			THEN 'short_haul'
		WHEN IFNULL(flight_range, 0) > 2500 AND IFNULL(flight_range, 0) <= 6000
			THEN 'medium-haul'
		WHEN IFNULL(flight_range, 0) > 6000
			THEN 'long-haul'
	END AS category
FROM airliners;